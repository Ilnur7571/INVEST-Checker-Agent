...
class Config:
    ...
    # Database - PostgreSQL на удаленном сервере
    DATABASE_URL = os.getenv("DATABASE_URL")  # Формат: postgresql://user:password@host:port/database
    ...
    def validate(cls):
        ...
        if not cls.DATABASE_URL:
            raise ValueError("DATABASE_URL не найден в .env - укажите строку подключения к PostgreSQL")
    ...
    def get_postgres_dsn(cls):
        """Получить DSN для PostgreSQL (для обратной совместимости)"""
