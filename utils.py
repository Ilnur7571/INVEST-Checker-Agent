...
def format_analysis_for_display(analysis: str, user_story: str = None) -> str:
    ...
    # Добавляем User Story если предоставлена
    if user_story:
        result = f"**User Story:**\n_{user_story}_\n\n{result}"
...
def estimate_token_count(text: str) -> int:
    ...
    if not text:
        return 0
    return len(text) // 2.5  # Консервативная оценка
...
def extract_acceptance_criteria(story: str) -> List[str]:
    """
    Извлекает критерии приемки из User Story.
    """
    criteria = []

    # Ищем паттерны критериев приемки
    patterns = [
    ...
    for pattern in patterns:
        ...
        if match:
            ...
            # Разбиваем на отдельные критерии
            criteria = re.split(r'\n\s*[•\-]\s*|\n\s*\d+\.\s*', criteria_text)
...
def extract_problems_for_display(analysis_text: str) -> str:
    """Извлекает проблемы для отображения в компактном формате"""
    if not analysis_text:
        return ""
    ...
    # Ищем невыполненные критерии
    for short, full in criteria_mapping.items():
        if f"{short}: ✗" in analysis_text or f"{full}: ✗" in analysis_text:
            problems.append(short)

    return ", ".join(problems) if problems else "все критерии выполнены"
