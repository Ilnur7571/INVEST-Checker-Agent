...
_user_story_cache: Dict[str, bool] = {}
...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        bot = context.bot_data['bot']
        stats_data = await bot.get_bot_stats()
        ...
        if 'error' in stats_data:
            ...
            stats_text += (
                ...
                f"• 📈 Общее использование: {stats_data.get('total_usage', 0)}\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Обработчик текстовых сообщений с улучшенной валидацией и кэшированием"""
    try:
        ...
        bot = context.bot_data['bot']
        cached_analysis = await bot.get_cached_analysis(text)
...
def _is_valid_user_story(text: str) -> bool:
    """Проверка валидности User Story с улучшенной логикой"""
    if len(text) > 2000:  # увеличили лимит
        return False

    # Проверка кэша
    cache_key = text.lower().strip()
    if cache_key in _user_story_cache:
        return _user_story_cache[cache_key]
    ...
    # Сохраняем в кэш
    _user_story_cache[cache_key] = is_valid
    if len(_user_story_cache) > 1000:
        _user_story_cache.clear()

    return is_valid

async def _analyze_user_story(update: Update, context: ContextTypes.DEFAULT_TYPE,
                             user_story: str, show_add_to_db: bool = False,
                             is_improved: bool = False, is_callback: bool = False) -> None:
    """Анализ User Story с исправленной обработкой ответа от LLM"""
    try:
        ...
        # Ищем похожие истории с улучшенным алгоритмом
        similar = await db.find_similar(user_story, threshold=0.85)

        # Если нашли ИДЕНТИЧНЫЕ истории (100%), используем их без LLM
        exact_matches = [s for s in similar if s[2] == 1.0]

        if exact_matches and not is_improved:
            # Берем самую релевантную идентичную историю
            ...
            # Форматируем ответ
            formatted_answer = format_analysis_for_display(answer, story)

            response_text = (
                f"🎯 **Найдена идентичная история в базе** (100% совпадение)\n\n"
                f"_{story}_\n\n{formatted_answer}"
            )
            ...
            bot.add_to_user_history(user_id, story, formatted_answer)
            await bot.cache_analysis(story, {
                'story': story,
        ...
        # Если есть очень похожие истории (выше 95%), но не идентичные
        high_similarity_matches = [s for s in similar if s[2] >= 0.95 and s[2] < 1.0]

        if high_similarity_matches and not is_improved:
            # Показываем похожие истории для выбора
            keyboard = similar_stories_keyboard(high_similarity_matches, user_story)
            if is_callback:
                await update.callback_query.edit_message_text(
                    "🔍 **Найдены очень похожие истории в базе:**\n\nВы можете выбрать одну из них или продолжить с вашим вариантом:",
                    reply_markup=keyboard
                )
            else:
                await update.message.reply_text(
            ...
            context.user_data['similar_stories'] = high_similarity_matches
            context.user_data['original'] = user_story
            return

        # Если есть просто похожие истории
        elif similar and not is_improved:
            keyboard = similar_stories_keyboard(similar, user_story)
            if is_callback:
                await update.callback_query.edit_message_text(
            ...
            context.user_data['similar_stories'] = similar
            context.user_data['original'] = user_story
            return

        else:
            # Только если нет похожих в базе - идем в LLM
            analysis_msg = None
            if is_callback:
            ...
            try:
                ...
                # Правильно извлекаем ответ из кортежа
                if isinstance(response, tuple) and len(response) >= 1:
                    answer_text = response[0]  # Берем первый элемент кортежа
                else:
                    answer_text = str(response)  # На всякий случай
                ...
                # Сохраняем в историю и кэш
                bot.add_to_user_history(user_id, user_story, analysis)
                await bot.cache_analysis(user_story, {
                    'story': user_story,
                    'analysis': analysis,
                    'timestamp': datetime.now().isoformat()
                })
    ...
    try:
        ...
        if data == "analyze_invest":
            ...
            if not original:
                ...
                return
            await _analyze_user_story(update, context, original, show_add_to_db=True, is_callback=True)
